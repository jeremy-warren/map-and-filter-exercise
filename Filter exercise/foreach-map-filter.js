...
function doubleValues(arr){
    let doubledValues = arr.map(t => t*2);
...
function onlyEvenValues(arr){
    let evenValues = arr.filter(function(t){
        return t % 2 == 0;})
    return evenValues;
}
...
function showFirstAndLast(arr){
    let fAndLArray = arr.map(function(t){
        let shortName = `${t[0]}${t[t.toString().length-1]}`;
        return shortName;
    })
    return fAndLArray;
}
...
function vowelCount(str){
   ...
   strArray.forEach(function(i){
       ...
       if (vowels.indexOf([lowercased]) != -1){
        vowelMap[i] ? vowelMap[i] ++ : vowelMap[i] = 1;
    } 
   })
...
/*
Write a function called valTimesIndex which accepts an array and returns a new array with each value multiplied by the index 
...
function valTimesIndex(arr){
    let output = arr.map(function(v,i){
        return v*i;
    });
    return output;
}
...
/*
Write a function called extractFullName which accepts an array of objects and returns a new array with the value of the key 
with a name of "first" and the value of a key with the name of  "last" in each object, concatenated together with a space. 
...
function extractFullName(arr){
    let fullName = arr.map(function(i){
...
/*
Write a function called filterByValue which accepts an array of objects and a key and returns a new array with all the objects 
...
function filterByValue(arr, key) {
    let output = arr.filter(function(val){
        if (val[key]){
            return val;}
    })
    return output;
...
function find(arr, searchValue) {
    let output = arr.filter(function(pj){
        if (pj == searchValue){return pj}
...
function removeVowels(str) {
    ...
    const vowels = 'aeiou';
    let output = lowercased.filter(function(letter){
...
function doubleOddNumbers(arr) {
    let onlyOdds = arr.filter(function(a){
        return (a % 2 != 0)
    })
    let doubledOdds = onlyOdds.map(function(b){
        return b*2;
    })
    return doubledOdds;
}
